...
class ChargingControlSwitchEntity(SwitchEntity):
        ...
        if ac_charge_entity_id:
            try:
                await self.hass.services.async_call(
                    "switch", "turn_on", {"entity_id": ac_charge_entity_id}
                )
    ...
    async def async_turn_off(self, **kwargs):
        ...
        if ac_charge_entity_id:
            try:
                await self.hass.services.async_call(
                    "switch", "turn_off", {"entity_id": ac_charge_entity_id}
                )
    ...
    async def reset_charge_plan(self):
        """Reset the charge plan to its initial state."""
        # Clear or reset relevant data for charge plan
        self.hass.data[DOMAIN]["slot_times"] = []  # Clear slot times
        self.hass.data[DOMAIN]["total_cost"] = 0  # Reset total cost
        self.hass.data[DOMAIN]["required_slots"] = 0  # Reset required slots

        # Log reset action
        _LOGGER.info("Charge plan has been reset.")

        # Call async_update on the relevant sensor entity
        charge_plan_sensor = self.hass.data[DOMAIN].get("charge_plan_sensor")
...
async def async_setup_entry(
    ...
    switch_entity = ChargingControlSwitchEntity(
        hass, "Charging Control Switch", update_charge_plan
    )
